...
template <uint32_t SHAPE_M, uint32_t SHAPE_N, uint32_t BLOCK_M, uint32_t BLOCK_N, uint32_t BLOCK_K, uint32_t kNumStages,
          uint32_t kNumLastStages, uint32_t kNumTMAThreads, uint32_t kNumMathThreadsPerGroup, uint32_t kNumTMAMulticast,
          bool kIsTMAMulticastOnA>
__global__ void __launch_bounds__(get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
    fp8_wgrad_gemm_per_tensor_kernel(uint32_t shape_k, const __grid_constant__ CUtensorMap tensor_map_a,
...
#pragma unroll
    ...
    auto scheduler =
        Scheduler<GemmType::Normal, SHAPE_N, BLOCK_M, BLOCK_N, 1, kNumTMAMulticast, kIsTMAMulticastOnA>(SHAPE_M);
...
#pragma unroll
                    for (uint32_t s = 0; s < kNumInnerStages; ++s) {
                        ...
                        tma_copy(&tensor_map_a, reinterpret_cast<uint64_t *>(&full_barrier), smem_a[s], k_idx,
                                 m_block_idx * BLOCK_M, num_tma_multicast_a);

                        // Issue TMA B
                        tma_copy(&tensor_map_b, reinterpret_cast<uint64_t *>(&full_barrier), smem_b[s], k_idx,
                                 n_block_idx * BLOCK_N, num_tma_multicast_b);
...
#pragma unroll
                ...
                tma_copy(&tensor_map_d, reinterpret_cast<uint64_t *>(&full_barrier), smem_d, n_block_idx * BLOCK_N,
                         m_block_idx * BLOCK_M, 1);
