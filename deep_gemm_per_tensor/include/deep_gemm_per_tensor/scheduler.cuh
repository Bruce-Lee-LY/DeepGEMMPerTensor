...
enum class GemmType { Normal, GroupedContiguous, GroupedMasked };

#pragma clang diagnostic push
#pragma ide diagnostic ignored "cppcoreguidelines-pro-type-member-init"
template <GemmType kGemmType, uint32_t SHAPE_N, uint32_t BLOCK_M, uint32_t BLOCK_N, uint32_t kNumGroups,
          uint32_t kNumTMAMulticast, bool kIsTMAMulticastOnA, uint32_t kNumNBlocks = ceil_div(SHAPE_N, BLOCK_N),
...
struct Scheduler {
    ...
    __device__ __forceinline__ explicit Scheduler(const uint32_t &shape_m, int *grouped_layout = nullptr) {
    ...
    __device__ __forceinline__ uint32_t get_global_idx(const uint32_t &shape_dim, const uint32_t &block_size,
        ...
        if constexpr (kGemmType == GemmType::Normal) {
            ...
            auto offset =
                kIgnoreGroupedForGroupedContiguous ? 0 : max(0, __ldg(grouped_layout + m_block_idx * BLOCK_M));
    ...
    __device__ __forceinline__ bool get_next_block(uint32_t &m_block_idx, uint32_t &n_block_idx) {
        ...
        if constexpr (kGemmType == GemmType::GroupedMasked) {
            ...
            is_peer_cta_alive =
                kNumNBlocks % kNumTMAMulticast == 0 or           // Always aligned on N (constant bypass)
