...
namespace deep_gemm_per_tensor {

template <uint32_t kNumFormerIters, uint32_t kGap, uint32_t kEnd>
__device__ __host__ void outer_launch_k_iterations(const auto &inner_launch_k_iterations, const auto &func,
    ...
    if constexpr (kNumFormerIters + kGap <= kEnd)
        outer_launch_k_iterations<kNumFormerIters + kGap, kGap, kEnd>(inner_launch_k_iterations, func,
                                                                      num_former_iters);
}

template <uint32_t SHAPE_N, uint32_t SHAPE_K, uint32_t BLOCK_M, uint32_t BLOCK_N, uint32_t BLOCK_K,
          uint32_t BLOCK_N_PADDING, uint32_t kSwizzleDMode, uint32_t kNumGroups, uint32_t kNumStages,
          uint32_t kNumTMAThreads, uint32_t kNumMathThreadsPerGroup, uint32_t kNumTMAMulticast, bool kIsTMAMulticastOnA,
          GemmType kGemmType>
__global__ void __launch_bounds__(get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
    fp8_gemm_per_tensor_kernel(int *grouped_layout, uint32_t shape_m, const __grid_constant__ CUtensorMap tensor_map_a,
    ...
    // Shared memory
    static constexpr bool kMustUseUniformed = (BLOCK_K % BLOCK_N == 0);
...
#pragma unroll
    ...
    auto launch_k_iterations = [](const auto &func, bool skip_computation, uint32_t num_former_iters) {
        constexpr bool kShouldOptimize = BLOCK_K / constexpr_gcd(BLOCK_K, BLOCK_N) <= 4 and not kMustUseUniformed;
                ...
                launch_k_iterations(
                    [&](uint32_t k_iter, auto divisible_type, auto _, auto __) {
                        ...
                        const uint32_t num_tma_multicast_a =
                            (kIsTMAMulticastOnA and is_tma_multicast_valid) ? kNumTMAMulticast : 1;
                        const uint32_t num_tma_multicast_b =
                            (not kIsTMAMulticastOnA and is_tma_multicast_valid) ? kNumTMAMulticast : 1;
...
#pragma unroll
                        for (uint32_t s = 0; s < kNumInnerStages; ++s) {
                            ...
                            tma_copy(&tensor_map_a, reinterpret_cast<uint64_t *>(&full_barrier), smem_a[s], k_idx,
                                     scheduler.get_global_idx(shape_m, BLOCK_M, m_block_idx), num_tma_multicast_a);
...
#pragma unroll
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            if constexpr (not kMustUseUniformed) {
...
#pragma unroll
                            for (uint32_t k = 0; k < BLOCK_K / WGMMA::K; ++k) {
                                auto desc_a = make_smem_desc(
                                    smem_a[s] + (math_wg_idx * WGMMA::M + m_offset) * BLOCK_K + k * WGMMA::K, 1);
...
#pragma unroll
                            for (uint32_t i = 0; i < WGMMA::kNumAccum / 4; ++i) {
                                shifted_accum[i * 4 + 0] += accum[i * 4 + 0];
                                shifted_accum[i * 4 + 1] += accum[i * 4 + 1];
                                shifted_accum[i * 4 + 2] += accum[i * 4 + 2];
                                shifted_accum[i * 4 + 3] += accum[i * 4 + 3];
                            }
                        }
                    }
...
#pragma unroll
                for (auto i = 0; i < WGMMA::kNumAccum / 4; ++i) {
                    ...
                    if constexpr (kSwizzleDMode > 0) {
                        ...
                        smem_ptr = reinterpret_cast<uint8_t *>(
                            smem_d + (m_offset + warp_idx * WGMMA_M_PER_WARP + lane_idx) * (BLOCK_N + BLOCK_N_PADDING) +
                            i * 8);
                    }
                    ...
                    SM90_U32x2_STSM_N<nv_bfloat162>::copy(
                        ...
                        __float22bfloat162_rn({shifted_accum[i * 4 + 2], shifted_accum[i * 4 + 3]}), smem_ptr);
                }
            }
            ...
            if (threadIdx.x < BLOCK_N / TMA_D_BLOCK_N) {
                ...
                cute::SM90_TMA_STORE_2D::copy(&tensor_map_d, smem_ptr, n_block_idx * BLOCK_N + in_block_n_offset,
