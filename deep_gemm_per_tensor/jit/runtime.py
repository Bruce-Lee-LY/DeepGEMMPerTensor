...
class Runtime:
    ...
    def __call__(self, **kwargs) -> cbd.CUresult:
        ...
        if self.kernel is None:
            ...
            illegal_names = ['vprintf', '__instantiate_kernel',
                             '__internal', '__assertfail']

            def check_illegal(line): return any(
            ...
            assert len(
                kernel_names) == 1, f'Too many kernels in the library: {kernel_names}'
        ...
        if use_cache and os.path.exists(path) and Runtime.is_path_valid(path):
            ...
            if name and (int(os.getenv('DG_JIT_DEBUG', 0)) or int(os.getenv('DG_PRINT_CONFIGS', 0))):
                ...
                for key, value in kwargs.items() if kwargs is not None else dict().items():
                    value = f'torch.Tensor<{value.dtype}>' if isinstance(
                        value, torch.Tensor) else value
                    value = f'cuda.bindings.driver.CUtensorMap' if isinstance(
                        value, cbd.CUtensorMap) else value
