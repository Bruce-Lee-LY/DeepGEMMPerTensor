...
def modify_segment(m, name, ffma_lines):
    ...
    for i in range(num_lines // 2):
        ...
        low_hex, high_hex = extract_hex_from_line(
            low_line), extract_hex_from_line(high_line)
        le_bytes.append(low_hex.to_bytes(8, 'little') +
        ...
        new_le_bytes.append(low_hex.to_bytes(8, 'little') +
                            high_hex.to_bytes(8, 'little'))
    ...
    offsets = list(filter(lambda x: validate(
        m, x, le_bytes, num_lines), offsets))
