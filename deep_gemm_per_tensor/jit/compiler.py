...
def get_deep_gemm_version() -> str:
    ...
    include_dir = os.path.join(get_jit_include_dir(), 'deep_gemm_per_tensor')
...
def get_default_user_dir():
    if 'DG_JIT_CACHE_DIR' in os.environ:
        ...
        return path
    return os.path.join(os.path.expanduser('~'), '.deep_gemm_per_tensor')
...
def put(path, data):
    ...
    tmp_file_path = os.path.join(
        make_tmp_dir(), f'file.tmp.{str(uuid.uuid4())}.{hash_to_hex(path)}')
...
class Compiler:
    ...
    def build(cls, name: str, code: str, runtime_cls: Type[Runtime], kwargs: Dict[str, Any] = None) -> Runtime:
        ...
        tmp_cubin_path = os.path.join(
            make_tmp_dir(), f'nvcc.tmp.{str(uuid.uuid4())}.{hash_to_hex(cubin_path)}.cubin')
...
class NVCCCompiler(Compiler):
    ...
    def flags(cls) -> List[str]:
        cxx_flags = ['-fPIC', '-O3', '-fconcepts',
                     '-Wno-deprecated-declarations', '-Wno-abi']
...
def build(name: str, code: str, runtime_cls: Type[Runtime], kwargs: Dict[str, Any] = None) -> Runtime:
    compiler_cls = NVRTCCompiler if int(
        os.getenv('DG_JIT_USE_NVRTC', 0)) else NVCCCompiler
