...
def make_2d_tma_a_desc(gemm_type: GemmType, t: torch.Tensor,
    ...
    return make_2d_tma_desc(t,
                            shape_k, shape_m *
                            (num_groups if gemm_type ==
                             GemmType.GroupedMasked else 1), m_stride,
                            block_k, block_m)
...
def make_2d_tma_b_desc(gemm_type: GemmType, t: torch.Tensor,
    ...
    return make_2d_tma_desc(t,
                            shape_k, shape_n *
                            (num_groups if gemm_type !=
                             GemmType.Normal else 1), n_stride,
                            block_k, block_n)
...
#include <deep_gemm_per_tensor/nvrtc_std.cuh>
#else
#include <cuda.h>
#include <string>
#endif
...
#include <deep_gemm_per_tensor/fp8_gemm_per_tensor.cuh>

using namespace deep_gemm_per_tensor;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&fp8_gemm_per_tensor_kernel<
        {kwargs['N']},
        {kwargs['K']},
        {kwargs['BLOCK_M']},
        {kwargs['BLOCK_N']},
...
#include <deep_gemm_per_tensor/fp8_wgrad_gemm_per_tensor.cuh>

using namespace deep_gemm_per_tensor;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&fp8_wgrad_gemm_per_tensor_kernel<
        {kwargs['M']},
        {kwargs['N']},
        {kwargs['BLOCK_M']},
        {kwargs['BLOCK_N']},
