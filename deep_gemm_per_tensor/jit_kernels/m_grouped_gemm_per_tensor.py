...
def m_grouped_gemm_per_tensor_fp8_fp8_bf16_nt_contiguous(lhs: torch.Tensor, rhs: torch.Tensor,
                                                         out: torch.Tensor, m_indices: torch.Tensor) -> None:
    """
    Perform a grouped GEMM (contiguous format) with FP8 inputs and BF16 output.
    ...
    Arguments:
        lhs: an FP8 tensor (typed `torch.float8_e4m3fn`) of shape `[m_sum, k]`.
        rhs: an FP8 tensor (typed `torch.float8_e4m3fn`) of shape `[num_groups, n, k]`.
    ...
    tensor_map_d = make_2d_tma_d_desc(
        GemmType.GroupedContiguous, out, m, n, n, block_m, block_n, num_groups, smem_config[1])
    ...
    runtime = build('m_grouped_gemm_fp8_fp8_bf16_nt',
                    code, FP8GemmRuntime, kwargs)
    runtime(**kwargs)


def m_grouped_gemm_per_tensor_fp8_fp8_bf16_nt_masked(lhs: torch.Tensor, rhs: torch.Tensor,
                                                     out: torch.Tensor, masked_m: torch.Tensor, expected_m: int) -> None:
    """
    Perform a grouped GEMM (masked format) with FP8 inputs and BF16 output.

    Requirements:
        LHS, RHS, and output tensors must be in contiguous format.
        RHS is required to be transposed.
        Moreover, this alignment requirement is different with the contiguous-format kernel, as we require that each batch
            should be separately transposed.

    Arguments:
        lhs: an FP8 tensor (typed `torch.float8_e4m3fn`) of shape `[num_groups, m_max, k]`.
        rhs: an FP8 tensor (typed `torch.float8_e4m3fn`) of shape `[num_groups, n, k]`.
    ...
    tensor_map_d = make_2d_tma_d_desc(
        GemmType.GroupedMasked, out, m, n, n, block_m, block_n, num_groups, smem_config[1])
