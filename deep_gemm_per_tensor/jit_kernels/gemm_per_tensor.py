...
def is_tma_multicast_legal(shape_dim: int, block_dim: int, num_tma_multicast: int, num_sms: int,
    ...
    divisible = ceil_div(
        shape_dim, block_dim) % num_tma_multicast == 0 or not require_divisible
...
def get_best_configs(m: int, n: int, k: int, num_groups: int, num_sms: int,
    ...
    block_ns = tuple(range(16, 129, 8)) + \
        ((136, 152, ) if is_wgrad else (144, 160, ))
    ...
    for num_stages in stage_candidates:
        best_smem_config = get_smem_config(
            num_stages, k, best_block_m, best_block_n, is_fp32_out=is_fp32_out, is_wgrad=is_wgrad)
...
def gemm_per_tensor_fp8_fp8_bf16_nt(lhs: torch.Tensor, rhs: torch.Tensor, out: torch.Tensor) -> None:
    """
    Perform a normal GEMM with FP8 inputs and BF16 output.
    ...
    runtime = build('gemm_per_tensor_fp8_fp8_bf16_nt',
